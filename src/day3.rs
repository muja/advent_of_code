...
pub fn part1(input: &str) -> usize {
    ...
    fabric
        .iter()
        .map(|x| x.iter().filter(|&&e| e > 1).count())
        .sum()
}
...
impl FromStr for RgxClaim {
    ...
    fn from_str(line: &str) -> Result<Self, Self::Err> {
        ...
        let id: isize = captures
            .get(1)
            .ok_or("capture id")?
            .as_str()
            .parse()
            .map_err(|_| "parse id")?;
        let x: usize = captures
            .get(2)
            .ok_or("capture x")?
            .as_str()
            .parse()
            .map_err(|_| "parse x")?;
        let y: usize = captures
            .get(3)
            .ok_or("capture y")?
            .as_str()
            .parse()
            .map_err(|_| "parse y")?;
        let w: usize = captures
            .get(4)
            .ok_or("capture w")?
            .as_str()
            .parse()
            .map_err(|_| "parse w")?;
        let h: usize = captures
            .get(5)
            .ok_or("capture h")?
            .as_str()
            .parse()
...
pub fn part2_unoptimized(input: &str) -> isize {
    ...
    'outer: for Claim(id, x, y, w, h) in all {
        for i in 0..w {
            for j in 0..h {
                if fabric[x + i][y + j] != 1 {
                    continue 'outer;
