...
pub fn part1(input: &str) -> i32 {
  ...
  let (id, minutes) = grid.into_iter()
    .max_by_key(|(_, v1)|
      v1.into_iter().map(|(id, m)| m - id).sum::<i32>()
    )
    .unwrap();
  ...
  frequency.iter().enumerate().max_by_key(|&(_, e)| e).expect("max minute").0 as i32 * id
}

#[aoc(day4, part2)]
pub fn part2(input: &str) -> i32 {
  let grid = guard_asleep_table(input);
  let (id, minute, _) = grid.iter()
    .map(|(k, v)| {
      let mut frequency = [0i32; 60];
      for &(start, end) in v {
        for i in start..end {
      ...
      let (minute, &times) = frequency.iter()
        .enumerate()
        .max_by_key(|&(_, e)| e)
        .expect("max minute");
      (k, minute, times)
    }).max_by_key(|&(_, _, times)| times).expect("max times");
  id * minute as i32
}
...
fn guard_asleep_table(input: &str) -> HashMap<i32, Vec<(i32, i32)>> {
  ...
  vec.into_iter().scan((-1, -1), |state, line| {
    match &line[19..20] {
      "G" => {
        // new Guard
        let id = line.split_at(26).1
          .chars()
      ...
      "w" => {
        ...
        guard_grid.entry(state.0).or_default().push((state.1, wakes_at_minute));
      },
      _ => {
